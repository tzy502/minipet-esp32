...
### 3.3 性别过滤（已废止，2026-09-26 用户拍板）

~~按 id 千位过滤~~ → **不分性别，发型/脸型全量展示**（与桌面版 2026-08-16 最终行为一致——
桌面版注释：「去除性别过滤（用户要求——男女角色不限制，发型/脸型全量展示）」）。
千位表（发型 0/3/5/6 男…）已从服务端移除；`gender` 字段仅作为外观草稿属性随配置下发，
不再影响目录列表。验收项「36633 不被误杀」在无过滤下自然成立。
